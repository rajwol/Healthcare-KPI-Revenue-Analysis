...
SELECT 
    ...
    SUM(billing_amount) AS CY_billing_amount,
    LAG(SUM(billing_amount)) OVER (ORDER BY DATEPART(YEAR, date_of_admission)) AS PY_billing_amount,
    SUM(billing_amount) - LAG(SUM(billing_amount)) OVER (ORDER BY DATEPART(YEAR, date_of_admission)) AS Variance
FROM healthcare_dataset
GROUP BY DATEPART(YEAR, date_of_admission);

--CY, PY, and Variance (Billing per visit)
SELECT 
    DATEPART(YEAR, date_of_admission) AS Year,
    AVG(billing_amount) AS CY_billing_per_visit,
    LAG(AVG(billing_amount)) OVER (ORDER BY DATEPART(YEAR, date_of_admission)) AS PY_billing_per_visit,
    AVG(billing_amount) - LAG(AVG(billing_amount)) OVER (ORDER BY DATEPART(YEAR, date_of_admission)) AS Variance
FROM healthcare_dataset
GROUP BY DATEPART(YEAR, date_of_admission);

--CY, PY, and Variance (Unique Patients)
SELECT 
    DATEPART(YEAR, date_of_admission) AS Year,
...
--CY, PY, and Variance (Average LOS (Days))
SELECT 
    DATEPART(YEAR, date_of_admission) AS Year,
...
YoYBilling AS (
    SELECT 
        ...
        y1.total_billing - y2.total_billing AS yoy_change
